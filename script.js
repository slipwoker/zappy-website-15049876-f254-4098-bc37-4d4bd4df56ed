  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, rule) {
      const label = field.closest('.form-group, .field-wrapper, div')
        ? (field.closest('.form-group, .field-wrapper, div').querySelector('label') || {}).textContent || field.name || field.placeholder
      ...
      const messages = {
        ...
        minlength: '\u064a\u062c\u0628 \u0623\u0646 \u064a\u0643\u0648\u0646 \u0647\u0630\u0627 \u0627\u0644\u062d\u0642\u0644 \u0623\u0637\u0648\u0644'
      };

      return messages[rule] || '\u064a\u0631\u062c\u0649 \u0645\u0631\u0627\u062c\u0639\u0629 \u0647\u0630\u0627 \u0627\u0644\u062d\u0642\u0644';
    }
    ...
    function validateField(field) {
      ...
      if (isValid && field.value.trim() !== '') {
        ...
        if (isValid && field.hasAttribute('minlength')) {
          ...
          if (!VALIDATORS.minlength(field.value, minLength)) {
            isValid = false;
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '15049876-f254-4098-bc37-4d4bd4df56ed',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '15049876-f254-4098-bc37-4d4bd4df56ed';
...
(function () {
  const lightbox = document.getElementById('glaLightbox');
  const lightboxImg = document.getElementById('glaLightboxImg');
  const lightboxCaption = document.getElementById('glaLightboxCaption');
  const lightboxClose = document.getElementById('glaLightboxClose');
  const lightboxBackdrop = document.getElementById('glaLightboxBackdrop');
  ...
  function openLightbox(imgSrc, altText) {
    ...
    lightbox.classList.add('gla-lightbox--open');
    lightbox.focus();
    document.body.style.overflow = 'hidden';
  }

  function closeLightbox() {
    lightbox.classList.remove('gla-lightbox--open');
    document.body.style.overflow = '';
    lightboxImg.src = '';
  }

  // Attach click to each gallery item
  document.querySelectorAll('.gla-gallery-item').forEach(function (item) {
    item.addEventListener('click', function () {
      const img = item.querySelector('.gla-gallery-img');
      const titleEl = item.querySelector('.gla-gallery-caption-title');
      if (img) {
        openLightbox(img.src, titleEl ? titleEl.textContent : img.alt);
      }
    });
  ...
  document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape' && lightbox.classList.contains('gla-lightbox--open')) {
      closeLightbox();
    }
  });
})();
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '15049876-f254-4098-bc37-4d4bd4df56ed';
